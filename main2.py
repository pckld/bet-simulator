...
# Function to load a graph from the sidebar
def load_graph(graph_name):
    global selected_graph
    selected_graph = graph_name  # Set the selected graph
    all_balance_histories.clear()  # Clear previous simulations
    all_balance_histories.extend(saved_graphs[graph_name])  # Load saved graph history
    plot_balance_over_time(all_balance_histories)  # Plot loaded graph
...
canvas_widget.pack(side=tk.TOP, fill=tk.BOTH, expand=1)  # Pack the canvas widget
